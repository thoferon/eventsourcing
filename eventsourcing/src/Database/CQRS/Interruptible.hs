...
timingOutStreamFamily
  :: MonadIO m
  => T.NominalDiffTime -> fam -> m (InterruptibleStreamFamily m fam)
timingOutStreamFamily diff streamFamily = do
  now <- liftIO T.getCurrentTime
  tvar <- liftIO . STM.newTVarIO $ T.addUTCTime diff now
  pure InterruptibleStreamFamily
    { streamFamily = streamFamily
    , interrupt = interrupt tvar
    }

  where
    interrupt :: MonadIO m => STM.TVar T.UTCTime -> [a] -> m Bool
    interrupt tvar events = liftIO $ do
